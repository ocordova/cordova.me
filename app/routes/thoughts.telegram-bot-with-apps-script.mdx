...
[Google Apps Script](https://developers.google.com/apps-script) is a JavaScript-based scripting service for Google's products. With a few lines of code you can call the [Telegram Bot API](https://core.telegram.org/bots/api), and you don't need a server, SSL certificates, or port configuration.
...
Talk to [BotFather](https://telegram.me/BotFather) to create and set up the bot. He will also give you the authorization token.
...
Open [script.google.com/intro](https://script.google.com/intro) in your browser and sign in. Create a blank project and delete any code in the **Code.gs** file.
...
We need an application that can respond to POST requests from the Bot API. Google Apps Script lets us deploy our script as a [Web App](https://developers.google.com/apps-script/guides/web), so we can interpret those requests.

When a user or program sends our script an HTTP POST request, Apps Script runs the special callback function **doPost(e)**. We define that function in our script. The `e` argument is an event parameter that contains the request information. The Bot API sends the POST request as JSON, so we receive an object converted to a string. We parse the text content of the POST body `e.postData.contents` with [JSON.parse()](https://developer.mozilla.org/en/docs/Web/JavaScript/Reference/Global_Objects/JSON/parse).
...
The Bot API supports basic formatting, so we'll format the quote with quotation marks, a dash, and the author in bold.

> "No matter how cool your interface is, it would be better if there were less of it."
> **- Alan Cooper**

The returned value contains HTML `p` tags, which the Bot API does not support. We remove them with the JavaScript [replace()](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/String/replace) method and the [regular expression](https://en.wikipedia.org/wiki/Regular_expression) `/<(?:.|\n)*?>/gm` [(explanation)](https://regex101.com/r/lL7tC7/1). We also strip the trailing line break `\n` with `/\n/gm`.
...
That is a working Telegram bot built with Google Apps Script. The source code is on GitHub if you want to extend it.
